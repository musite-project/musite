...
class Router(object):
    ...
    """ A Router is an ordered collection of route->target pairs. It is used to
        ...
        the first target that satisfies the request. The target may be
        anything, usually a string, ID or callable object. A route consists of
        a path-rule and a HTTP method.
    ...
    def __init__(self, strict=False):
        ...
        self.filters = {
            ...
            'float': lambda conf: (
                r'-?[\d.]+', float, lambda x: str(float(x))
                ),
            'path': lambda conf: (r'.+?', None, None)}

    def add_filter(self, name, func):
        """Add a filter. The provided function is called with the configuration
        string as parameter and must return a (regexp, to_python, to_url)
        tuple. The first element is a string, the last two are callables or
        None. """
...
class Route(object):
    ...
    def __init__(self, app, rule, method, callback, name=None,
        ...
        self.method = method
        #: The original callback with no plugins applied.
        ...
        #: A list of plugins to not apply to this route
...
class Bottle(object):
    ...
    def mount(self, prefix, app, **options):
        """ Mount an application (:class:`Bottle` or plain WSGI) to a specific
            ...
            All other parameters are passed to the underlying :meth:`route`
            call.
        """
    ...
    def uninstall(self, plugin):
        """ Uninstall plugins. Pass an instance to remove a specific plugin, a
            type object to remove all plugins that match that type, a string to
            remove all plugins with a matching ``name`` attribute or ``True``
    ...
    def _cast(self, out, peek=None):
        ...
        # (recursive, because they may wrap anything)
        # TODO: Handle these explicitly in handle() or make them iterable.
        if isinstance(out, HTTPError):
            out.apply(response)
            out = self.error_handler.get(
                out.status_code, self.default_error_handler)(out)
    ...
    def wsgi(self, environ, start_response):
        ...
        try:
            ...
            start_response(
                '500 INTERNAL SERVER ERROR', headers, sys.exc_info())
            return [tob(err)]
...
class BaseRequest(object):
    ...
    def __init__(self, environ=None):
        ...
        #: This is the only real attribute.
    ...
    def query(self):
        """ The :attr:`query_string` parsed into a :class:`FormsDict`. These
            ...
            but not to be confused with "URL wildcards" as they are provided by
            the :class:`Router`. """
    ...
    def json(self):
        """ If the ``Content-Type`` header is ``application/json``, this
            ...
            requests smaller than :attr:`MEMFILE_MAX` are processed to avoid
            memory exhaustion. """
    ...
    def POST(self):
        ...
        self[
            '_cgi.FieldStorage'
        ] = data  # http://bugs.python.org/issue18394#msg207958
        data = data.list or []
        for item in data:
            if item.filename:
    ...
    def path_shift(self, shift=1):
        ...
        self['SCRIPT_NAME'], self[
            'PATH_INFO'] = path_shift(script, self.path, shift)
...
class BaseResponse(object):
    ...
    expires = HeaderProperty('Expires',
                             reader=lambda x: datetime.utcfromtimestamp(
                                 parse_date(x)),
    ...
    def set_cookie(self, name, value, secret=None, **options):
        """ Create a new cookie or replace an old one. If the `secret` parameter is
            ...
            :param expires: a datetime object or UNIX timestamp.
              (default: None)
            :param domain: the domain that is allowed to read the cookie.
              (default: current domain)
            :param path: limits the cookie to a given path
              (default: current path)
            :param secure: limit the cookie to HTTPS connections
                           (default: off).
            :param httponly: prevents client-side javascript to read this
              cookie (default: off, requires Python 2.6 or newer).

            If neither `expires` nor `max_age` is set (default), the cookie
            will expire at the end of the browser session (as soon as the
    ...
    def get(self, key, default=None, index=-1, type=None):
        """ Return the most recent value for a key.
            ...
            :param type: If defined, this callable is used to cast the value
                    into a specific type. Exception are suppressed and result
                    in the default value to be returned.
        """
        try:
...
class ConfigDict(dict):
    ...
    def update(self, *a, **ka):
        """ If the first parameter is a string, all keys are prefixed with this
            namespace.
            Apart from that it works just as the usual dict.update().
...
class FileUpload(object):
    ...
    def filename(self):
        """ Name of the file on the client file system, but normalized to
            ensure file system compatibility. An empty filename is returned
            as 'empty'.
        ...
        fname = normalize('NFKD', fname).encode(
...
def static_file(
    ...
    """ Open a file in a safe way and return :exc:`HTTPResponse` with status
        ...
        :param root: Root path for file lookups. Should be an absolute
            directory path.
...
def parse_auth(header):
    """ Parse rfc2617 HTTP authentication header string (basic)
        and return (user,pass) tuple or None"""
    try:
...
class RocketServer(ServerAdapter):
    ...
    def run(self, handler):
        ...
        server = Rocket(
            (self.host, self.port), 'wsgi', {'wsgi_app': handler})
        server.start()
...
class AutoServer(ServerAdapter):
    ...
    """ Untested. """
    adapters = [WaitressServer, PasteServer,
                TwistedServer, CherryPyServer, WSGIRefServer]
...
def load_app(target):
    ...
    try:
        ...
        default_app.remove(
            tmp)  # Remove the temporary added default application
        NORUN = nr_old
...
class BaseTemplate(object):
    ...
    def search(cls, name, lookup=None):
        ...
        if not lookup:
            # 0.12
            depr('The template lookup path list should not be empty.', True)
            lookup = ['.']
...
cheetah_template = functools.partial(
    template, template_adapter=CheetahTemplate)
